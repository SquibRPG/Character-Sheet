...
function showMove(moveOrId) {
 ...
 // const role = getPrimaryRole(activeSlot) || "striker";
  ...
  // Robust: many moves have null/empty descriptions in moves.json
  const damageForDescription = previewDamageText || damageText || "";
  const moveDescription = typeof move.description === "string" ? move.description : "";
  const moveNotes = typeof move.notes === "string" ? move.notes : "";
  const baseDesc =
  ...
  const descriptionHtml = baseDesc
    ? (prefix + baseDesc.replaceAll("{damage}", damageForDescription) + suffix)
    : "";


  const targetText = Array.isArray(move.target) ? move.target.join(", ") : (move.target ?? "—");
  const rangeText = move.range ?? "—";
  const economyText = move.economy ?? "—";
  const tagsText = Array.isArray(move.tags)
    ? (move.tags.length ? move.tags.join(", ") : "None")
    : (move.tags ? String(move.tags) : "None");
  ...
  const buffDebuffText = (() => {
    const bd = move.buffDebuff;
    if (!bd?.stat || bd.stat === "none") return "None";
    const amount = bd.amount === null || bd.amount === undefined ? "" : String(bd.amount);
    const duration = bd.duration ? ` (${bd.duration})` : "";
    const stat = String(bd.stat).toUpperCase();
    return `${stat}${amount ? " " + amount : ""}${duration}`.trim();
  })();
  ...
  box.innerHTML = `
    ...
    <em>${economyText} (${tierText}) · ${categoryText}</em><br>
    ${descriptionHtml ? `<br>${descriptionHtml}<br>` : ""}
    ...
    <strong>Tags:</strong> ${tagsText}<br><br>

    <strong>Status:</strong> ${statusText}<br>

  <strong>Buff/Debuff:</strong> ${buffDebuffText}
`;
